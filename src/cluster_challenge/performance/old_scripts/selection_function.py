...
matching_folder = '/sps/lsst/groups/clusters/cluster_comparison_project/after_matching/cosmoDC2_wazp.cosmoDC2.truez/v0_v0/proximity_matching/deltaz_0.05_matchradius_1.0mpc_pref_more_massive/'
...
##########select case
catalog1 = 'cosmoDC2.v0.fits'
catalog2 = 'wazp.cosmoDC2.truez.v0.fits'
##########

#outpath = "/pbs/home/t/tguillem/web/clusters/cluster_challenge/selection_function/matching/redmapper_cosmoDC2/m200c/"
outpath = '/sps/lsst/users/rsolomon/web/desc/cluster_comparison_project/cosmoDC2_wazp.cosmoDC2.truez/v0_v0/proximity_matching/deltaz_0.05_matchradius_1.0mpc_pref_more_massive/testing/selection_function/'
...
c_merged = ClCatalog.read(matching_folder+"cosmoDC2.v0_wazp.cosmoDC2.truez.v0_merged.fits", 'merged', tags=cltags)
...
##mass_density_metrics		## MASS-RICHNESS PLOT
#plt.figure()
...
##M200c vs halo_mas
#plt.figure()
#plt.scatter(c_merged['log_mass'], np.log10(c_merged['richness']), marker='.',color = 'blue', s=10, alpha=0.3, label='clusters')
##plt.xscale('log')
##plt.yscale('log')
#plt.xlim([13, 15])
#plt.ylim([0.0, 2.5])
...
#richness threshold
xbins = 10**np.linspace(13.5, 15, 20)
ybins = [25,35,50,70,100]
for i in range(0,n_zbins):
     cut1 = zbins[i]
     cut2 = zbins[i+1]
     filter1 = np.logical_and(c_merged['z_halo'] > cut1, c_merged['z_halo'] < cut2)
     c_merged_cut = c_merged[filter1]
     
     #halo_mass
     counts, xedges, yedges = np.histogram2d(c_merged_cut['mass'], c_merged_cut['richness'], bins=(xbins, ybins))
     fig, ax = plt.subplots()
     ax.pcolormesh(xbins, ybins, counts.T, cmap='jet')
     ax.set_yscale('log')
     ax.set_xscale('log')
     ax.set_ylabel('NGALS')
     ax.set_xlabel('halo_mass')
...
labels = ["m0", "F", "G"]
fig = pygtc.plotGTC(chains=flat_chains, paramNames=labels)
#fig = corner.corner(
#             flat_chains,
#             labels=labels,
#             color = 'blue',
